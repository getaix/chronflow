...
class TestRabbitMQBackendMocked:
    ...
    @pytest.mark.asyncio
    @patch("symphra_scheduler.backends.rabbitmq_backend.connect_robust", new_callable=AsyncMock)
    async def test_connect(self, mock_connect):
        """测试连接 RabbitMQ。"""
        from symphra_scheduler.backends.rabbitmq_backend import RabbitMQBackend
    ...
    @pytest.mark.asyncio
    @patch("symphra_scheduler.backends.rabbitmq_backend.connect_robust", new_callable=AsyncMock)
    @patch("symphra_scheduler.backends.rabbitmq_backend.Message")
