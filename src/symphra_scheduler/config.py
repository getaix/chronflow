...
class BackendConfig(BaseModel):
    ...
    def instantiate(self, *, default_queue_size: int | None = None) -> QueueBackend:
        ...
        示例:
            ...
            ```
        """
        from symphra_scheduler.backends import create_backend
...
class SchedulerConfig(BaseSettings):
    ...
    示例:
        ...
        # 从环境变量(symphra_scheduler_MAX_WORKERS=20)
        config = SchedulerConfig()
    """

    model_config = SettingsConfigDict(
        env_prefix="symphra_scheduler_",  # 环境变量前缀
        env_file=".env",  # 环境变量文件
    ...
    persistence_path: Path = Field(
        default=Path(".symphra_scheduler_state.json"),
    ...
    pid_file: Path = Field(
        default=Path("~/.symphra_scheduler/symphra_scheduler.pid"),
        description="守护进程 PID 文件路径",
    )
    process_name: str = Field(
        default="symphra-scheduler",
    ...
    def validate_backend_exists(cls, value: BackendConfig) -> BackendConfig:
        ...
        """
        from symphra_scheduler.backends import get_registered_backends
