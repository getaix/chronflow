...
使用示例:
    ```python
    from symphra_scheduler.backends import register_backend, create_backend
...
def _lazy_backend(module_path: str, attr: str) -> BackendFactory:
    ...
    参数:
        module_path: 后端模块的完整路径(如 "symphra_scheduler.backends.redis_backend")
    ...
    示例:
        ...
        register_backend(
            "redis",
            _lazy_backend("symphra_scheduler.backends.redis_backend", "RedisBackend")
        )
...
register_backend("redis", _lazy_backend("symphra_scheduler.backends.redis_backend", "RedisBackend"))
register_backend(
    "rabbitmq", _lazy_backend("symphra_scheduler.backends.rabbitmq_backend", "RabbitMQBackend")
)
...
try:  # pragma: no cover - 仅在依赖满足时导出
    from symphra_scheduler.backends.redis_backend import RedisBackend  # type: ignore
...
try:  # pragma: no cover
    from symphra_scheduler.backends.rabbitmq_backend import RabbitMQBackend  # type: ignore
