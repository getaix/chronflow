...
from symphra_scheduler.config import TaskMetrics
...
class TaskConfig(BaseModel):
    ...
    def get_next_run_time(
        ...
        # 检查任务是否在时间范围内
        if self.start_time:
            start = self.start_time if self.start_time.tzinfo is not None else self.start_time.replace(tzinfo=UTC)
            if now < start:
                return start

        if self.end_time:
            end = self.end_time if self.end_time.tzinfo is not None else self.end_time.replace(tzinfo=UTC)
            if now >= end:
                return None
        ...
        if self.schedule_type == ScheduleType.ONCE:
            ...
            # symphra_scheduler 支持秒级精度的 6 段式 Cron 表达式: 秒 分 时 日 月 周
